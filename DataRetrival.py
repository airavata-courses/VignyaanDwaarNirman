...
def callback(ch, method, properties, body):
    ...
    start_date = datetime.strptime(start_date[4:24], '%b %d %Y %I:%M:%S')
    end_date = data['end_date']
    end_date = datetime.strptime(end_date[4:24], '%b %d %Y %I:%M:%S')
    print("data retrival sdate: ", start_date)
    user_id = data['user_id']
    function_type = data['function_type']
    timestamp = datetime.fromtimestamp(time.gmtime()).strftime('%Y-%m-%d %H:%M:%S')
    
    #payload = {'function_type':function_type, 'radar_id':radar_id,'start':start_date,'end':end_date}
    payload = {"user_id":user_id, "function_type":function_type, "radar_id":radar_id, "start_date":str(start_date), "end_date":str(end_date),"timestamp":timestamp}
